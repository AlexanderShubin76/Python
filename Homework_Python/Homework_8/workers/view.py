...
def rerecord_id(): # С помощью данного метода изменяем данные сотрудника.
    ...
    last_name = input('Введите новую фамилию сотрудника. Если фамилию не нужно менять, нажмите "0": ')
    if last_name  == "0":
        last_name = md.workers[id]['last_name']
    post = input('Введите должность сотрудника. Если должность не нужно менять, нажмите "0": ')
    if post  == "0":
        post = md.workers[id]['post']
    tel_number = input('Введите телефонный номер сотрудника. Если телефон не нужно менять, нажмите "0": ')
    if  tel_number == "0":
